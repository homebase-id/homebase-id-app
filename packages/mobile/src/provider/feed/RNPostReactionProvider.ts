import { DEFAULT_PAYLOAD_KEY, DotYouClient, EmbeddedThumb, HomebaseFile, ImageContentType, NewHomebaseFile, PayloadFile, PriorityOptions, ReactionFile, ScheduleOptions, SecurityGroupType, SendContents, ThumbnailFile, TransferUploadStatus, uploadFile, UploadFileMetadata, UploadInstructionSet } from '@homebase-id/js-lib/core';
import { jsonStringify64, stringToUint8Array, getNewId, getRandom16ByteArray } from '@homebase-id/js-lib/helpers';

import { TransitInstructionSet, TransitUploadResult, uploadFileOverPeer } from '@homebase-id/js-lib/peer';
import { GetTargetDriveFromChannelId, ReactionConfig, ReactionContext } from '@homebase-id/js-lib/public';
...
export const saveComment = async (
    ...
    if (!shouldEmbedContent) {
        payloads.push({
            payload: new OdinBlob([payloadBytes], { type: 'application/json' }) as any as Blob,
