...
export const getThumbBytesOverPeer = async (
    ...
    const url = await encryptUrl(`${dotYouClient.getEndpoint()}/transit/query/thumb?${stringifyToQueryParams({ ...request, width, height, lastModified })}`, ss);
...
export const getPayloadBytesOverPeer = async (
    ...
    const ss = dotYouClient.getSharedSecret();
    if (!ss) throw new Error('Shared secret not found');
    const url = await encryptUrl(`${dotYouClient.getEndpoint()}/transit/query/payload?${stringifyToQueryParams({ ...request, lastModified })}`, ss);
