import { assertIfDefined, assertIfDefinedAndNotDefault, splitSharedSecretEncryptedKeyHeader, stringifyToQueryParams } from '@youfoundation/js-lib/helpers';
...
export const getPayloadBytesOverPeerByGlobalTransitId = async (
    ...
    const url = await encryptUrl(`${dotYouClient.getEndpoint()}/transit/query/payload_byglobaltransitid?${stringifyToQueryParams({ ...request, lastModified })}`, ss);
    ...
    return ReactNativeBlobUtil.config({
        ...
        if (
            ...
            res.info().headers.sharedsecretencryptedheader64 && decrypt
        ) {
...
export const getThumbBytesOverPeerByGlobalTransitId = async (
    ...
    const url = await encryptUrl(`${dotYouClient.getEndpoint()}/transit/query/payload_byglobaltransitid?${stringifyToQueryParams({ ...request, width, height, lastModified })}`, ss);
    return ReactNativeBlobUtil.config({
        fileCache: true,
    }).fetch('GET', url, {
        bx0900: authToken,
